...
class SingDataset(BaseDataset):
    ...
    def __getitem__(self, index):
        ...
        attn_prior = np.load(os.path.join(self.data_dir, spk, "attn_prior", fileid + '.npy'))
        return pho, pitchid, dur, slur, gtdur, mel, f0, wav, spkid, attn_prior
...
class SingCollate():
    ...
    def pad_3D(self, inputs, B, T, L):
        inputs_padded = np.zeros((B, T, L), dtype=np.float32)
        for i, input_ in enumerate(inputs):
            shape0 = np.shape(input_)[0]
            shape1 = np.shape(input_)[1]
            if shape0 > T:
                print("pad error!!!\nerror!!!\n")
            ...
            if shape1 > L:
                ...
                # print(inputs.shape, inputs_padded.shape)
                shape1 = L
            inputs_padded[i, :shape0, :shape1] = input_[:shape0, :shape1]

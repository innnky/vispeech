...
def train_and_evaluate(rank, epoch, hps, nets, optims, schedulers, scaler, loaders, logger, writers):
    ...
    for batch_idx, (phonemes, phonemes_lengths,
        ...
        arousal = arousal.cuda(rank, non_blocking=True)
        ...
        with autocast(enabled=hps.train.fp16_run):
            time_3 = time.time()
            y_hat, l_length, l_pitch,l_arousal, ids_slice, x_mask, z_mask, \
            (z, z_p, m_p, logs_p, m_q, logs_q), pred_f0, pred_arousal= net_g(phonemes, phonemes_lengths, f0, arousal, phndur,
        ...
        with autocast(enabled=hps.train.fp16_run):
            ...
            with autocast(enabled=False):
                ...
                loss_arousal = torch.sum(l_arousal.float())
                loss_mel = F.l1_loss(y_mel, y_hat_mel) * hps.train.c_mel
                loss_kl = kl_loss(z_p, logs_q, m_p, logs_p, z_mask) * hps.train.c_kl

                loss_fm = feature_loss(fmap_r, fmap_g)
                loss_gen, losses_gen = generator_loss(y_d_hat_g)
                loss_gen_all = loss_gen + loss_fm + loss_mel + loss_dur + loss_kl + loss_pitch + loss_arousal
        time_9 = time.time()
        optim_g.zero_grad()
        ...
        if rank == 0:
            if global_step % hps.train.log_interval == 0:
                ...
                losses = [loss_disc, loss_gen, loss_fm, loss_mel, loss_dur, loss_kl, loss_pitch,loss_arousal]
                ...
                scalar_dict.update(
                    {"loss/g/fm": loss_fm, "loss/g/mel": loss_mel, "loss/g/dur": loss_dur, "loss/g/kl": loss_kl,
                     "loss/g/pitch": loss_pitch,"loss/g/arousal":loss_arousal})
                ...
                image_dict = {
                    ...
                    "all/arousal": utils.plot_data_to_numpy(arousal[0, :].cpu().numpy(), pred_arousal[0, :].detach().cpu().numpy()),
                }
                utils.summarize(
                    writer=writer,
...
def evaluate(hps, generator, eval_loader, writer_eval):
    ...
    with torch.no_grad():
        for batch_idx, (phonemes, phonemes_lengths,
            ...
            for shift, arousal_shift in [(0.8, 1), (1, 0.5), (1, 1)]:
                ...
                y_hat, mask, xx, pred_f0 = generator.module.infer(phonemes, phonemes_lengths,
                                                         max_len=1000, sid=sid,shift=shift,arousal_shift=arousal_shift)
                ...
                audio_dict.update({
                    f"gen/audio-{batch_idx}-{shift}-{arousal_shift}": y_hat[0, :, :y_hat_lengths[0]]
                })

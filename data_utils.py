...
class TextAudioSpeakerLoader(torch.utils.data.Dataset):
    ...
    def _filter(self):
        ...
        for wav_path, phonemes, phn_dur, sid, phf0, pharousal in self.audiopaths_and_text:
            # get_size获取文件大小（字节数），这里计算wav的长度，根据上方计算公式得出结果
            lengths.append(os.path.getsize(wav_path) // (2 * self.hop_length))
            audiopath_and_text_new.append([wav_path, phonemes, phn_dur, sid, phf0, pharousal])
    ...
    def get_audio_text_pair(self, audiopath_and_text):
        ...
        wav_path, phonemes, phn_dur, spk, phf0, pharousal = audiopath_and_text
        ...
        arousal = torch.FloatTensor([float(i) for i in pharousal.strip().split(" ")])
        assert arousal.shape == f0.shape
        assert abs(spec.shape[-1] - sum(phn_dur))<2, wav_path
        assert phonemes.shape ==phn_dur.shape, phonemes
        assert phonemes.shape ==f0.shape
        return phonemes,f0, phn_dur, spec, wav, spk,arousal
...
class TextAudioSpeakerCollate():
    ...
    def __call__(self, batch):
        ...
        max_arousal_len = max([len(x[6]) for x in batch])
        ...
        arousal_padded = torch.FloatTensor(len(batch), max_arousal_len)
        ...
        for i in range(len(ids_sorted_decreasing)):
            ...
            arousal_padded[i, :arousal.size(0)] = arousal
        ...
        return  phonemes_padded, phonemes_lengths,f0_padded,arousal_padded,phndur_padded,\
